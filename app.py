...
if skicka:
    if (st.session_state.input_larar_id and st.session_state.input_amne and
        ...
        # Rensa formulärfälten på rätt sätt efter widget initiering
        for key in ["input_larar_id", "input_amne", "input_undervisningstid", "input_larar_klasser", "input_arbetsdagar", "input_onskemal"]:
...
if sal_submit and st.session_state.input_sal_namn:
    ...
    # Rensa sal-formulär på rätt sätt
    st.session_state.input_sal_namn = ""
    st.session_state.input_sal_klass = klasser[0]
    st.session_state.input_sal_amne = amnen[0]
    st.session_state.input_sal_typ = "Hemklassrum"
...
if "generated_schema" in st.session_state:
    ...
    if not vis_df.empty:
        vis_df = df[df[visningstyp.lower()] == val].sort_values(by=["dag", "start"])
